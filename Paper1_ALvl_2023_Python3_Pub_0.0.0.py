...
class Dastan: #Board Game
    def __init__(self, R, C, NoOfPieces): #Initialise the board with number of pieces, rows (R) and columns (C)
        self._Board = []
        self._Players = []
        self._MoveOptionOffer = []
        self._Players.append(Player("Player One", 1)) #player one is the first player
        self._Players.append(Player("Player Two", -1)) #player two is the last player
        self.__CreateMoveOptions()
        self._NoOfRows = R
        self._NoOfColumns = C
        self._MoveOptionOfferPosition = 0
        self.__CreateMoveOptionOffer()
        self.__CreateBoard() #Graphics rendering
